...
# 1:N - 한 엔티티가 여러 개와 관계(교수 - 강의, 학생 - 수강내역, 게시글 - 댓글)
# N:N - 여러 엔티티가 서로 다수와 관계(학생 - 과목: 중간 테이블 필요)

### 실무 ERD 표현 규칙 ###
# 1) 부모 PK를 자식이 FK로 가짐
#	> 실선 관계: 자식이 부모의 존재에 종속 (부모가 있어야만 자식이 존재 - PK + FK)
#	> 점선 관계: 자식이 독립적으로 존재 가능 (부모 없이도 자식 생성 가능 - FK)

# 2) 예시 1
# 학생과 수강 내역
# 학생 테이블) 부모 테이블 - 학번 PK 설정, 학생명 ... 등
...
# 주문 상세 내역 테이블) 자식 - 주문ID(FK), 주문 상품 ... 등
# >> 관계: 자식 테이블의 기본키가 부모의 기본키를 포함하거나 그대로 사용 (1:1 관계)

# 4) 예시 3
# 회원과 즐겨찾기 프로그램
# 회원 테이블) 부모 - 회원ID(PK), 이름 ... 등
# 즐겨찾기 프로그램 테이블) 자식 - 고유 번호ID(PK), 회원 번호(FK), 프로그램명 ... 등
# >> 비회원 또는 임시 사용자도 '즐겨찾기 기능'에 대해 허용
#		- 회원 없이도 자식 테이블이 존재 가능한 구조로 설계 가능(1:N관계 - 점선 관계)
...
create table course_enrollment ( # 강의 등록 테이블(수강)
    ...
    # >> stu_id와 course_id의 조합이 비워질 수 없으며, 둘을 합쳐서 유일한 값이 되어야 함
    foreign key (stu_id) references student(stu_id),
    foreign key(course_id) references course(course_id)
);

### MySQL Worckbench에서 ERD 다이어그램 생성 방법 ###
# 1. 테이블 생성
# 2. 상단 텍스트 메뉴바 [Database] > Reverse Engineer
