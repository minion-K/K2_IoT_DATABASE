...
/*
	SELECT
		컬럼명
        1) 여러 개 나열 시 ,(콤마)로 구분
        2) 모든 컬럼 조회 (* - 테이블 정의 시 작성되는 컬럼순으로 표시)
...
/*
	...
	cf) distinct: 해당 컬럼에서 중복되는 데이터를 제거하고 하나만 남김
*/
...
/*
	...
	3) NULL 체크: NULL 여부를 확인
		>> IS NULL, IS NOT NULL
        
	4) 범위 연산자: A 이상 B 이하 범위]
		>> BETWEEN A AND B (숫자형, 날짜형 데이터)
	...
	7) 집합 연산자: 결과 집합을 병합/비교
		>> UNION, INTERSECT, EXCEPT
		(MySQL은 UNION만 지원 - 합집합)
*/
USE `korea_db`;

-- 2024년 회원 중 이름에 '민'이 들어간 회원을 조회(name, area_code 조회)
-- +) name과 area_code 컬럼은 각각 '회원 이름', '지역 코드'로 컬럼명 조회
SELECT 
	name '회원이름', area_code '지역 코드'
FROM 
	`members`
WHERE
	YEAR(join_date) = 2024 
