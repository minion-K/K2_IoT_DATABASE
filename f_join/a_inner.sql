...
#	  employees 테이블 - salaies 테이블 (1명의 회원이 여러 급여 기록을 가질 수 있음)
...
FROM `purchases` AS P #별칭은 테이블 첫 문자를 대문자로 사용
	INNER JOIN `members` AS M
    ON P.member_id = M.member_id
WHERE P.member_id = 1;

# cf) JOIN 시 두개 이상의 테이블에서 동일한 열 이름이 존재하는 경우
#		>> 테이블명(별칭).열이름 형식으로 표기
# Error Code: 1052. Column 'member_id' in where clause is ambiguous (where절이 애매하다)
...
# 내부 조인 예제 4
# 구매 이력이 있는 회원만 조회
SELECT distinct M.name, M.contact # 조인한 테이블의 데이터가 반드시 조회될 필요는 없음
FROM `members` M
	JOIN `purchases` P
    ON M.member_id = P.member_id;
    
# cf) 서브쿼리 (구매 이력이 있는 회원 조회)
# : 위의 조인과 결과는 동일하지만 서브쿼리 테이블의 데이터조회는 불가
SELECT name, contact
FROM `members`
WHERE member_id IN (
