/*
    ...
    - () 안에 작성하여 필요한 값을 먼저 추출한 뒤 메인 쿼리에 
...
SELECT name, grade
FROM `members`
WHERE grade IN ('Diamond', 'Platinum', 'Gold', 'Silver');
...
# 차이점
# > 조건이 단순하고 고정 (서브쿼리 X): 빠르고 간단
# > 조건이 복잡하거나 동적으로 바뀔 수 있음 (서브쿼리 O): 더 유연하고 안전 - NOT IN

SELECT * FROM `members`; 
SELECT * FROM `purchases`; # 구매 ID(PK), 구매 회원 ID(FK), 상품 코드, 구매일, 가격, 수량

# -- 서브쿼리가 반드시 필요한 예시 --
# 1) 구매한 이력이 있는 있는 회원을 제외한 회원 (구매 이력이 없는 회원 조회)
SELECT member_id, name
FROM `members`
WHERE member_id NOT IN (
	-- 서브쿼리 값이 동적으로 변화될 수 있음
...
# >> puchases에서 회원별 구매 총액을 계산하고 가장 높은 사람 1명만 추출
