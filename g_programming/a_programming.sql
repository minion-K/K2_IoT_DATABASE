...
# cf) SQL 변수 특징
# 1) MySQL Workbench 시작 시 유지, 종료할 경우 사라짐(임시 저장 공간)
# 2) SQL 비절차적 언어: 원하는 부분을 따로따로 실행 (변수 선언문 실행 >> 변수 출력문 실행)
...
# >> ? 키워드를 사용하여 코드 작성 시 값이 채워지지 않지만, 실행 시에는 채워지는 코드 작성 가능and
