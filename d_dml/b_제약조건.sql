...
/*
    ...
    cf) 유일성 제약
        ...
        > 테이블의 특성을 반영한 열 선택을 권장 || PK 컬럼을 따로 생성 권장
...
SELECT * FROM `students`; # 테이블 내부의 실제 데이터 확인
...
# 기존테이블의 기본 키 제약 조건 삭제 시 not null에 대한 조건은 사라지지 않음
DESC `students`;
...
/*
    ...
    > 고객이 실제로 존해하는지 확인, 고객과 주문 간의 관계 명시
*/
...
create table `orders` (
	...
	# 외래 키 지정 방법
    # foreign key (참조컬럼, 현 테이블의 테이블명) references 기본테이블(키본컬럼, PK컬럼)
...
# (`example`.`orders`, CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`member_id`) REFERENCES `members` (`member_id`)
...
# 1) 제약 조건 이름 검색 필수
# where 조건 내에서 컬럼명을 데이터처럼 사용; 따옴표 지정
# > 문법 그 자체로 인식
...
desc `orders`;
# > MUL: 외래 키 지정시 MySQL이 자동으로 인덱스(index) 생성
#		- 외래 키 삭제 후에도 존재
