...
insert into `integer`
values (128, 32767, 2000000000, 90000000000000000); 
# Out of range value for column 'T_COL' at row
# > Out of range 오류: 해당 데이터 타입의 범위를 벗어나는 경우 발생alter

# cf) unsigned: 부호가 없는 정수
# : 각 숫자형 데이터 타입의 시작을 0부터 가지는 옵션
# - 범위는 그대로 인식(EX. tinyint: 0 ~ 256)
...
/*
	...
	+) 다량의 텍스트 데이터 형식
		- text 형식: 1 ~ 65000 byte (blob)
        - longtext 형식: 1 ~ 약 42억 byte (longblob)
...
values ('Laptop Samsung Book 3 pro', '전자제품', '삼성 갤럭시 북3 노트북 프로', 'example');
...
create table product (
    ...
    price1 float(7,2),		-- float, double 의 형태로만 사용권장
    -- price1 float (권장)
    price2 double(10, 4),
	-- price2 double (권장)
