...
/*
    ...
    : 어떤 속성 집합이 다른 속성을 '결정(함수 종속)' 할 수 있다면
		, 해당 속성 집합을 '결정자' (Key는 아님)
	> '학번', '주민 번호' 들 대부분의 후보키가 결정자가 될 수 있음
*/

# 3NF는 비주요 속성이 후보키가 아니어도 허용
# : BCNF는 모든 결정자가 반드시 후보키(유일성, 최소성)여야만 함
...
# 1NF(원자값)
# 2NF(완전 함수 종속) - 기본키 일부에만 종속된 속성 제거
# 3NF(이행 종속 제거) - 이행 종속 제거 (A > B > C 제거)
