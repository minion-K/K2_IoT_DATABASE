/*
    ...
    4. group by 그룹화할 컬럼명: 특정 열을 기준으로 그룹화
    5. having 그룹조건: 그룹화 된 데이터에 대한 조건 지정
    6. order by 정렬 컬럼명: 결과를 특정 컬럼의 순서로 정렬
    7. limit 컬럼 수 제한: 반환할 행(레코드)의 수를 제한
    
    cf) DB 내부 실제 실행 순서
    FROM 테이블 > JOIN(추가 테이블 데이터 가져오기) > WHERE(필터링)
    > group by(그룹화) > having(조건) > select(컬럼 추출) > order by(정렬) > limit(제한)
*/
...
# cf) null 값은 값이 없음을 나타냄: 그 어떤 값과도 비교, 연산이 불가
...
select * from `members`
where
	points between 200 and 400; -- 이상, 이하의 개념
...
# 시작이 J 이고 뒤는 0개 이상의 문자를 허용: J%
select * from `members`
where
	name like 'J%';
    
# 시작이 J이고 뒤는 3개의 임의의 문자를 허용: J___
select * from `members`
where
	name like 'J___';
    
# 어떤 문자열 내에서든 'un'이 포함만 되면 허용: %un%
select * from `members`
where
	name like '%un%';
    
# 한개의 임의의 문자 뒤에 u가 0개 이상의 임의의 문자 허용: _u%
